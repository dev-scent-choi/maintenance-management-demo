...
const ProjectEditModal: React.FC<Props> = ({ isOpen, onClose, onSaved, project, preloadedUsers, preloadedCompanies }) => {
  ...
  useEffect(() => {
    ...
    const managerId = project.ownerId || '';
    const memberIds = (project.memberIds || []).filter((id: string) => id !== managerId);
    setFormData({
      name: project.name || '',
  ...
  const handleSubmit = async () => {
    if (isSubmitting || !formData.name.trim() || !formData.startDate || !user || !token || !project) return;
    ...
    try {
      ...
      const statusLabels: Record<string, string> = {
        planning: '계획', in_progress: '진행 중', on_hold: '보류', completed: '완료', cancelled: '취소',
      };
      const changes: { field: string; oldValue: string; newValue: string }[] = [];
      if (formData.name !== project.name)
        changes.push({ field: '프로젝트명', oldValue: project.name || '', newValue: formData.name });
      if (formData.description !== (project.description || ''))
        changes.push({ field: '설명', oldValue: project.description || '(없음)', newValue: formData.description || '(없음)' });
      if (formData.status !== project.status)
        changes.push({ field: '상태', oldValue: statusLabels[project.status] || project.status, newValue: statusLabels[formData.status] || formData.status });
      if (formData.companyId !== (project.companyId || ''))
        changes.push({ field: '업체', oldValue: project.companyId || '(없음)', newValue: formData.companyId || '(없음)' });
      if (formData.startDate !== toDateInput(project.startDate))
        changes.push({ field: '시작일', oldValue: toDateInput(project.startDate) || '(없음)', newValue: formData.startDate || '(없음)' });
      if (formData.dueDate !== toDateInput(project.dueDate))
        changes.push({ field: '마감일', oldValue: toDateInput(project.dueDate) || '(없음)', newValue: formData.dueDate || '(없음)' });
      if (formData.managerId !== (project.ownerId || '')) {
        const oldPM = users.find(u => u.id.toString() === project.ownerId);
        const newPM = users.find(u => u.id.toString() === formData.managerId);
        changes.push({ field: '담당자', oldValue: oldPM?.name || project.ownerId || '(없음)', newValue: newPM?.name || formData.managerId || '(없음)' });
      }

      addLog({ userId: user.id, userName: user.name, action: 'update', resourceType: 'project', resourceId: project.id, resourceName: formData.name, details: `프로젝트가 수정되었습니다: ${formData.name}`, changes: changes.length > 0 ? changes : undefined });
      onSaved();
      onClose();
    } catch (e) {
      console.error(e);
      ...
      >
        ...
        >
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            <div>
              <p style={{ fontSize: '0.75rem', color: themeColors.textSecondary }}>프로젝트</p>
              <span style={{ fontSize: '0.9375rem', fontWeight: 700, color: themeColors.text, lineHeight: 1.2 }}>{project?.name || '프로젝트 수정'}</span>
            </div>
          </div>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '20px', display: 'flex', flexDirection: 'column', gap: '12px', minHeight: 0 }}>
            ...
            <div style={{ backgroundColor: isDark ? 'rgba(255,255,255,0.03)' : 'rgba(55,53,47,0.03)', borderRadius: '4px', border: `1px solid ${themeColors.border}`, padding: '14px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginBottom: '12px' }}>
                <span style={{ color: themeColors.textSecondary, fontSize: '0.6875rem', fontWeight: 600, letterSpacing: '0.06em', textTransform: 'uppercase' }}>기본 정보</span>
              </div>
        ...
        <div style={{
          ...
          <button
            ...
            disabled={!formData.name.trim() || !formData.startDate || isSubmitting}
            style={{
              display: 'flex', alignItems: 'center', gap: '6px',
              padding: '6px 14px', borderRadius: '4px',
              backgroundColor: themeColors.primary, color: onPrimaryText,
              fontSize: '0.8125rem', fontWeight: 600,
              opacity: (!formData.name.trim() || !formData.startDate || isSubmitting) ? 0.5 : 1,
              cursor: (!formData.name.trim() || !formData.startDate || isSubmitting) ? 'not-allowed' : 'pointer',
              border: 'none',
            }}
          >
            <Save size={14} />
